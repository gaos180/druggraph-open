...
import { toolsApi, DiseaseGnnResult, DiseasePrediction } from '../../api/tools';
...
// Enlace a la ficha de la enfermedad en su ontología (MONDO/EFO/Orphanet/HP vía OLS/EBI).
function diseaseHref(id: string): string | null {
  if (!id) return null;
  const curie = id.replace('_', ':');
  return `https://www.ebi.ac.uk/ols4/search?q=${encodeURIComponent(curie)}`;
}

export default function DiseaseGnnTool() {
  usePageTitle('Repurposing fármaco→enfermedad (GNN)');
  const [drugId, setDrugId] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [result, setResult] = useState<DiseaseGnnResult | null>(null);

  const handleSearch = async () => {
    const id = drugId.trim().toUpperCase();
    if (!id) { setError('Ingresa un DrugBank ID'); return; }
    setLoading(true); setError(''); setResult(null);
    try { const res = await toolsApi.diseaseGnn(id); setResult(res.data); }
    catch (err: any) {
  ...
  const preds: DiseasePrediction[] = result?.predictions || [];
  const m = result?.model || {};

  const exportCsv = () => {
    if (!preds.length) return;
    const rows = preds.map(p => [p.disease_id, `"${p.disease_name}"`, p.probability].join(','));
    const blob = new Blob([['disease_id,disease_name,probability', ...rows].join('\n')], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = `disease_gnn_${drugId}.csv`; a.click();
  ...
  return (
    <div className="max-w-5xl">
      <HandTitle className="text-2xl flex items-center gap-2"><Stethoscope className="w-6 h-6 text-amber-800" /> Repurposing fármaco→enfermedad</HandTitle>
      <p className="text-stone-500 text-[13px] font-hand mt-1 mb-5">
        Enfermedades a las que un fármaco podría reposicionarse, predichas por una GNN de enlaces sobre el knowledge graph (Drug↔Target↔Disease, embeddings FastRP + Link Prediction) — enfoque estilo BiomedGPS. El modelo reporta su AUCPR en test.
      </p>
      ...
      <div className={cardCls}>
        <div className="flex gap-3 items-end flex-wrap">
          ...
          <PencilButton variant="solid" onClick={handleSearch} disabled={loading}>{loading ? 'Consultando…' : 'Predecir enfermedades'}</PencilButton>
      ...
      {result?.available && (
        <>
          <div className={`${cardCls} flex gap-5 items-center flex-wrap`}>
            ...
            <div className="text-center"><div className="text-2xl font-bold font-hand text-purple-700">{preds.length}</div><div className="text-stone-500 text-xs">Enfermedades</div></div>
            {m.auc_pr !== undefined && (
            ...
            {preds.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-xs">
                  <thead><tr className="border-b border-stone-800/10">{['#', 'Enfermedad', 'ID', 'Probabilidad'].map((h, i) => <th key={i} className={thc}>{h}</th>)}</tr></thead>
                  <tbody>
                    {preds.map((p, i) => {
                      const href = diseaseHref(p.disease_id);
                      return (
                        <tr key={i} className={`border-b border-stone-800/5 ${i % 2 ? 'bg-stone-500/5' : ''}`}>
                          <td className="px-2 py-1.5 text-stone-500">{i + 1}</td>
                          <td className="px-2 py-1.5 text-stone-800">{p.disease_name || '—'}</td>
                          <td className="px-2 py-1.5 font-mono text-sky-700">
                            {href ? <a href={href} target="_blank" rel="noreferrer" className="hover:underline">{p.disease_id}</a> : (p.disease_id || '—')}
                          </td>
          ...
          <div className="text-stone-400 text-[11px] font-hand mb-4">
            Hipótesis de reposicionamiento in silico (no confirmadas). El AUCPR mide la calidad del modelo en un test held-out.
          </div>
        </>
      )}
    </div>
