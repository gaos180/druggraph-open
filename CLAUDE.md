...
- **Tier 5 (estructural)** — inicio: modelado de pharmacóforos (ver `docs/TIER5_PLAN.md`, `docs/PHARMACO_SUITE_INTEGRATION.md`):
  ...
  - `config/services/docking_service.py` (5.3) — **docking estructural con AutoDock Vina**: `dock(smiles, target)` prepara el ligando a PDBQT (RDKit+**Meeko**) y lo acopla al receptor preparado (**OpenBabel** → PDBQT + caja del sitio activo) devolviendo afinidad (kcal/mol) + poses. Receptores en `backend/models/docking/<target>/` (gitignoreado; prepara con `scripts/prepare_receptor.py`). `dock_for_drug(id, target)`, `list_targets()`. Deps `vina`+`meeko`+`gemmi`+`openbabel-wheel`. `DockingUnavailable`→503.
