...
- **Tier 5 (estructural)** — inicio: modelado de pharmacóforos (ver `docs/TIER5_PLAN.md`, `docs/PHARMACO_SUITE_INTEGRATION.md`):
  ...
  - `config/services/denovo_pharma_service.py` (5.2) — de novo **guiado por pharmacóforo**: GA sobre **SELFIES** (todo decodifica a SMILES válido) con fitness = match al pharmacóforo objetivo · QED − 0.05·SA + filtro de diversidad. Se expone como `engine='pharma'` de `denovo_service`. Deps `selfies`+RDKit. 503 si faltan.
  - `config/services/docking_service.py` (5.3) — **docking estructural con AutoDock Vina**: `dock(smiles, target, ph=)` prepara el ligando a PDBQT (RDKit+**Meeko**, protonado al `ph` con dimorphite-dl) y lo acopla al receptor devolviendo afinidad (kcal/mol) + poses. `ensure_receptor(uniprot)` prepara receptores **on-demand desde AlphaFold** (por subprocess — OpenBabel co-cargado con vina segfaultea) → acopla a dianas ELEGIDAS/predichas (caja ciega, `blind=True`). `dock_many` (mapas una vez, cribado batch), `screen_results` (lee `docking_results`), `dock_for_drug`, `list_targets`. Deps `vina`+`meeko`+`gemmi`+`openbabel-wheel`+`dimorphite-dl`. `DockingUnavailable`→503.
- **Tier 6 (homología cross-especies / veterinaria):**
  - `config/services/homology_service.py` — para saber si un fármaco funcionaría en OTRAS especies: busca los **ortólogos** de sus dianas (humanas) en las especies elegidas (UniProt por gen+organismo) y mide la **% de identidad** (alineamiento global BLOSUM62 con Biopython). ≥70% ⇒ diana conservada ⇒ probable que el fármaco funcione. `conservation(genes, species_ids)`, `homology_for_drug(id, species_ids)`. 11 especies preset (perro/gato/caballo/vaca…). `POST /api/tools/homology/` + `GET /api/tools/homology/species/`. Frontend `HomologyTool` en `/tools/homology`. Deps `biopython`+`requests`. 503 sin biopython.
  - `config/services/md_service.py` (5.3 fase 2) — refinamiento de poses por **Dinámica Molecular** (OpenMM + OpenFF/SMIRNOFF): `refine(smiles, target)` acopla, construye el complejo, minimiza + MD corta y reporta energías/estabilidad. `POST /api/tools/docking/refine/`. REQUIERE conda (openff-toolkit no está en pip) → `MD_OK`/503 sin él.
  - `config/services/molecule_analysis_service.py` — **panel integral** (`/api/tools/molecule-analysis/`): agrega propiedades/vecinos/espacio químico/pharmacóforo/ADMET + red. Fuera del catálogo, la red usa el **consenso de los 3 vecinos** (dianas/enfermedades compartidas = más probables).
...
- **Tier 4 (ML propio) scripts:**
  ...
  - `scripts/prepare_receptor.py` (5.3) — descarga un PDB, elimina aguas, prepara el receptor rígido en PDBQT (OpenBabel) y define la caja del sitio activo (ligando co-cristalizado / metal / coords) → `backend/models/docking/<target>/`. Ej: `--pdb-id 3SPU --target ndm1 --center-metal Zn --chain A`.
